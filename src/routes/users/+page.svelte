<script lang="ts">
    ...
    let searchInput = '';
    $: dynamicUsers = data.users.filter(user => (
        user.cardId.includes(searchInput) ||
...
<div class="flex flex-col py-4 px-8 w-full h-full">
    <div class="h-12">
        <Input autocomplete="off" id="search" placeholder="Cerca" size="md" bind:value={searchInput} class="focus:border-secondary-button focus:ring-0">
